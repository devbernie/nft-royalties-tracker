...
def mock_env_vars(monkeypatch):
    ...
    monkeypatch.setenv("BASE_URL", "https://preview.koios.rest/api/v1")
...
def test_get_policy_assets_success(mocker, mock_policy_id):
    mock_response = [
        {"asset": "asset1", "policy_id": {"value": mock_policy_id}},
        {"asset": "asset2", "policy_id": {"value": mock_policy_id}}
    ]
...
def test_get_metadata_invalid_id(mocker, mock_transaction_id):
    mock_response = []
    mocker.patch("requests.post", return_value=mocker.Mock(status_code=200, json=lambda: mock_response))
    metadata = get_metadata(mock_transaction_id)
    assert metadata == {}